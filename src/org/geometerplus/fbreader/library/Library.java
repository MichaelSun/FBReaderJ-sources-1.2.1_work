...
import org.amse.ys.zip.ZipUtil;
...
import org.geometerplus.zlibrary.ui.michaeldoupo.R;
...
//		int res = R.raw.book1;
//		String outFileName = "book1.epub";
//		Context context = ZLAndroidApplication.Instance();
//        final String apppath = context.getFilesDir().getAbsolutePath();
//        final String userpath = apppath + "/book/";
//        
//        File file = new File(userpath + outFileName);
//        if (!file.exists()) {
//            extractRawFile(context, res, outFileName, userpath);
//        }

//		ZLFile ret = ZLFile.createFileByPath(userpath + outFileName);
		ZLFile ret = ZLFile.createFileByPath("book/book.epub");
		
		return ret;
	}

    private static void extractRawFile(Context context, int dictId, String dictName, String targetPath) {
        Resources res = context.getResources();
        try {
            InputStream is = res.openRawResource(dictId);
            ZipUtil.outputFile(is, targetPath, dictName);
            is.close();
            is = null;
