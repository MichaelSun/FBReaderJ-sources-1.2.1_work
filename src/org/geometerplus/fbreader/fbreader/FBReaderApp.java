...
public final class FBReaderApp extends ZLApplication {
	...
	public void initWindow() {
		...
		if ((book != null) && book.File.exists()) {
		    final Book readBook = book;
            wait("loadingBook", new Runnable() {
                public void run() {
                    openBookInternal(readBook, null);
                }
            });
		} else {
		    Log.d("FBReaderAPP", "[[getInitBook]]");
            wait("loadingBook_forfirst", new Runnable() {
                public void run() {
                    Book book = Book.getByFile(Library.getHelpFile());
